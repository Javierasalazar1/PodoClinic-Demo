...
- Logo de clínica opcional (puede funcionar sin logo).
- Panel de recordatorios de próximas visitas.
- Gestión de cuenta propia del administrador (correo y contraseña).
- Archivado de pacientes (soft delete, solo ADMIN).
...
1. El usuario hace clic en "Olvidé mi contraseña" en la pantalla de login.
2. Ingresa su correo electrónico registrado.
3. El sistema envía un email con un enlace de recuperación de un solo uso (expira en 1 hora).
4. El enlace lleva a una pantalla donde el usuario ingresa y confirma su nueva contraseña (mín. 8 caracteres, al menos una mayúscula, un número y un símbolo).
5. El enlace se invalida inmediatamente al ser usado, aunque no haya expirado.
6. Se registra el evento en `audit_logs` con la IP del dispositivo.
7. Se envía un email de confirmación notificando que la contraseña fue cambiada (con opción de reportar si no fue el usuario).

> **Instrucción para la IA:** implementar tabla `password_reset_tokens` con campos `id`, `user_id`, `token_hash`, `expires_at`, `used_at` (nullable). Nunca almacenar el token en claro, solo su hash SHA-256.
...
- **Logo (opcional):** imagen PNG/JPG/SVG, max 2MB, almacenada en bucket S3. Si no se sube logo, el nombre de la clínica en texto reemplaza visualmente al logo en la interfaz y en el PDF. Debe existir un botón "Eliminar logo" para volver al estado sin logo.
...
#### Gestión de cuenta del administrador (Mi cuenta)

El administrador puede modificar sus propios datos desde una sección "Mi cuenta" separada de la configuración de clínica:

- **Cambiar correo electrónico:** requiere ingresar la contraseña actual para confirmar. Se envía email de verificación al nuevo correo antes de hacer el cambio efectivo. El correo anterior recibe notificación del cambio.
- **Cambiar contraseña:** requiere ingresar la contraseña actual + nueva contraseña + confirmación. La nueva contraseña debe cumplir política de seguridad (mín. 8 caracteres, mayúscula, número, símbolo). Al cambiar la contraseña, se revocan todos los refresh tokens activos (cierre de sesión en todos los dispositivos).
- Ambas acciones se registran en `audit_logs`.

### 3.3 Módulo de gestión de especialistas

- El administrador **agrega** especialistas con: nombre completo, título profesional, número de colegiación, email, foto de perfil (opcional). El sistema envía un email de bienvenida al especialista con enlace para establecer su contraseña.
- El administrador puede **editar** los datos de cualquier especialista (nombre, título, colegiación, foto).
- El administrador puede **desactivar** una cuenta de especialista (`is_active = false`). El especialista no puede iniciar sesión pero sus historiales clínicos se conservan íntegros.
- El administrador puede **reactivar** una cuenta previamente desactivada.
- Cada especialista ve solo los pacientes que él mismo ha atendido (aislamiento de datos).
- La lista de especialistas muestra: nombre, título, estado (activo/inactivo), fecha de creación y número de consultas realizadas.
...
#### Archivado de pacientes (soft delete)

- **Solo el ADMIN** puede archivar un paciente.
- Archivar un paciente lo oculta de todas las listas y búsquedas normales, pero **no elimina ningún dato** de la base de datos (ni consultas, ni consentimientos, ni fotografías).
- En la tabla `patients` se agrega el campo `archived_at: DateTime?`. Si tiene valor, el paciente está archivado.
- El ADMIN puede ver pacientes archivados desde un filtro especial "Ver archivados" en la lista de pacientes.
- El ADMIN puede **desarchivar** un paciente (restaurarlo a activo).
- **No existe eliminación física de pacientes** en la interfaz. El borrado real de la BD solo ocurre por solicitud legal documentada, ejecutada manualmente por el administrador del sistema.
- Al intentar archivar un paciente, el sistema muestra un diálogo de confirmación: "Este paciente será ocultado de las listas. Sus historiales clínicos se conservarán. ¿Continuar?"
- La acción de archivar/desarchivar se registra en `audit_logs`.
...
### 3.6 Módulo de recordatorios de próximas visitas

Este módulo es una vista del dashboard que permite al especialista y al administrador gestionar los seguimientos pendientes de pacientes.

#### Vista del panel de recordatorios

- Accesible desde el menú principal como "Recordatorios" o desde el dashboard como widget.
- Muestra una lista de pacientes que tienen fecha de próxima consulta registrada, ordenados por proximidad de la fecha.
- Filtros disponibles: **Próximos 7 días** / **Próximos 15 días** / **Próximos 30 días** / **Vencidos** (fecha pasada sin nueva consulta registrada).
- Cada fila muestra: nombre del paciente, teléfono, fecha de próxima consulta, especialista asignado, y botones de acción.

#### Acciones disponibles por fila

- **Enviar recordatorio por WhatsApp:** abre WhatsApp Web con el número del paciente y un mensaje pre-escrito editable: *"Hola [Nombre], le recordamos que tiene una cita en [Nombre Clínica] el [Fecha]. Para consultas llámenos al [Teléfono clínica]."*
- **Enviar recordatorio por correo:** envía email al paciente con el mismo mensaje usando el SMTP de la clínica.
- **Marcar como contactado:** registra que se realizó el recordatorio (campo `last_reminder_sent_at` en la tabla `patients`).
- **Crear nueva consulta:** acceso directo al formulario de nueva consulta para ese paciente.

#### Reglas de visibilidad

- El **ADMIN** ve todos los recordatorios de todos los especialistas.
- El **ESPECIALISTA** solo ve los recordatorios de sus propios pacientes.
- El rol **RECEPCIÓN** puede ver el panel de recordatorios y enviar mensajes, pero no puede acceder a los datos clínicos del paciente.

#### Campos requeridos en la base de datos

En la tabla `patients` agregar:
- `next_visit_date: DateTime?` — fecha sugerida de próxima consulta (se actualiza automáticamente al finalizar una consulta si el especialista la indicó en el plan de tratamiento).
- `last_reminder_sent_at: DateTime?` — última vez que se envió un recordatorio.

> **Instrucción para la IA:** la fecha `next_visit_date` en `patients` debe sincronizarse automáticamente cuando el especialista finaliza una consulta y ha completado el campo "Fecha sugerida próxima consulta" en la Sección 9 del formulario. Esto se hace en el endpoint `POST /consultations/:id/finalize`.
...
model Patient {
  ...
  photo_url               String?
  next_visit_date         DateTime? // fecha sugerida próxima consulta, sincronizada al finalizar consulta
  last_reminder_sent_at   DateTime? // última vez que se envió recordatorio al paciente
  archived_at             DateTime? // null = activo; con valor = archivado (soft delete)
  created_at              DateTime @default(now())
  updated_at              DateTime @updatedAt
}
```
...
model AuditLog {
  ...
  user_id       String?
  clinic_id     String?
  action        String   // LOGIN, LOGOUT, CREATE_CONSULTATION, FINALIZE_CONSULTATION, GENERATE_PDF, SEND_EMAIL, CHANGE_PASSWORD, CHANGE_EMAIL, ARCHIVE_PATIENT, UNARCHIVE_PATIENT, SEND_REMINDER, ADD_SPECIALIST, DEACTIVATE_SPECIALIST, UPDATE_CLINIC_LOGO, DELETE_CLINIC_LOGO, etc.
...
model PasswordResetToken {
  id          String    @id @default(uuid())
  user_id     String
  token_hash  String    // hash SHA-256 del token enviado por email, nunca el token en claro
  expires_at  DateTime
  used_at     DateTime? // null si no ha sido usado; se rellena al usarlo
  created_at  DateTime  @default(now())
}
```
...
| POST | `/change-password` | Recibe `{current_password, new_password}`. Revoca todos los refresh tokens activos al completar. Requiere auth. |
| POST | `/change-email` | Recibe `{current_password, new_email}`. Envía verificación al nuevo correo antes de hacer el cambio efectivo. Requiere auth. |
| GET | `/verify-email-change/:token` | Confirma el cambio de correo electrónico con el token del email. No requiere auth. |
...
| POST | `/logo` | Sube nuevo logo (multipart/form-data). Solo `ADMIN`. El logo es opcional; la clínica puede operar sin logo. |
| DELETE | `/logo` | Elimina el logo actual. La interfaz y el PDF usan el nombre de la clínica como texto en su lugar. Solo `ADMIN`. |
...
| PATCH | `/:id/archive` | Archiva el paciente (soft delete: `archived_at = now()`). Solo `ADMIN`. |
| PATCH | `/:id/unarchive` | Restaura un paciente archivado (`archived_at = null`). Solo `ADMIN`. |
| GET | `/archived` | Lista pacientes archivados. Solo `ADMIN`. |
...
### 6.6 Recordatorios — `/api/v1/reminders`

| Método | Ruta | Descripción |
|---|---|---|
| GET | `/` | Lista pacientes con próxima visita. Query params: `?range=7` (días). `ADMIN` ve todos; `SPECIALIST` solo los suyos. |
| GET | `/overdue` | Lista pacientes con fecha de próxima visita vencida y sin nueva consulta registrada. |
| POST | `/:patientId/send-whatsapp` | Genera enlace de WhatsApp con mensaje pre-escrito y actualiza `last_reminder_sent_at`. |
| POST | `/:patientId/send-email` | Envía email de recordatorio al paciente y actualiza `last_reminder_sent_at`. |
| PATCH | `/:patientId/mark-contacted` | Actualiza `last_reminder_sent_at` sin enviar mensaje (para marcar contacto manual). |
...
- Autenticación completa (login, 2FA, refresh tokens, olvidé mi contraseña).
- CRUD de pacientes + archivado de pacientes (soft delete).
- Formulario de consulta (secciones 1-4 y 9).
- Generación de PDF básico (con y sin logo de clínica).
- Descarga directa del PDF.
- Gestión de cuenta del administrador (cambiar correo y contraseña).
- Gestión de especialistas (agregar, editar, activar/desactivar).
...
- Panel de recordatorios de próximas visitas.
- Envío de recordatorios por WhatsApp y correo desde el panel.
...
*Fin del documento de especificación — PodoClinic v1.1*
